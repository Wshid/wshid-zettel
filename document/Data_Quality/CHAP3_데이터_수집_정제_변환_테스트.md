...
# 3.5. 데이터 정규화
- 첫 번째 운영 데이터 변환 레이어 -> 데이터 정규화 단계
	- 이름은 조직에 따라 다름
- 데이터 변환
	- 하나 이상의 소스 형식 -> 목적지 형식 으로 데이터를 이동하는 프로그램
- 정규화는 데이터가 **파이프라인**을 통과하는 과정에서 첫 번째로 겪는 변환
- 노이즈, 모호성, 이질성이 최대치인 진입점 데이터에서
	- 정규화가 발생하기 때문에 특별히 고려해야할 점이 있음

## 3.5.1. 이종 데이터 소스 처리
- 서로 다른 데이터 소스에서 데이터 수집
	- 일부는 유용하나, 대부분 쓸모가 없음
- 파이프라인의 정규화 지점에서 데이터를 설명하는 몇 가지 주요 기능

### 대기 시간에 최저화
- 스트리밍 엔드포인트 데이터 -> 생성 즉시 사용할 수 있도록 최적화
- 고정된 네트워크 성능을 고려하여 처리 가능
	- 실제로는 데이터의 완전성을 결정함
- 최종 상태와 관계없이 파이프라인으로 즉시 푸시
	- **데이터 배치는 불완전한 것으로 예상**

### 비위계적인 형식
- 데이터 표준화를 지향하라면
	- 데이터는 효율성과 사용 편의성을 위해
	- 비위계적인, **수평적인** 스토리지 형식으로 저장
- 깨끗한 [[data_warehouse|데이터 웨어하우스]] + 스키마 + 테이블 저장 방식 보다는
	- S3 버킷과 같은 일부 중앙 저장소에 데이터를 한번에 덤핑할 가능성이 높음

### 원시 파일 형식
- 진입점 데이터는 수평적 뿐만 아니라
	- 스트리밍된 위치에서 **원래 파일 형식**을 반영할 수 있음
- app 로그 데이터, 센서 데이터를 표 형식 변화 x
	- 대부분의 데이터는 이러한 변환이 필요 x
	- 비용도 많이 듦

### 선택적 데이터 필드
- [[data_warehouse|데이터 웨어하우스]]와 달리 JSON과 같은 원시 파일 데이터는 **선택적 필드**를 가질 수 있음
- 해당 필드의 부재가 의미하는 것이 `NULL`값인지, 숫자 `0`인지 추론해야 할 수 있음
- 해당 필드에 따라 기본값이 될 수 있으며
	- 해당 필드가 없는 경우 upstream 처리에 문제가 될 수도, 아닐수도 있음

### 이질성
- 위의 모든 특징은 특정한 종류의 이질성을 가리킴
- 데이터는 다양한 원본 파일 형식으로 다양한 소스로부터 제공되며
	- 동일한 형식의 이전 데이터와 비교하여 처리된 양이 다를 수 있음
- 예측 가능한 종류의 이질성에 대비하여, 데이터를 이해하는 방법을 배워야 함
- 일단 데이터가 저장되고 처리되면, 데이터를 쉽게 변환하여 최대한의 영향을 줄 수 있도록 보장

#### 이질성 측면: [[data_warehouse|데이터 웨어하우스]] vs [[data_lake|데이터 레이크]]
- 앞의 기능들은 대부분 [[data_lake|데이터 레이크]]방식으로 데이터를 설명함
- 레이크는 수용할 수 있는 데이터 유형의 제약이 훨씬 적기 때문에
	- 진입점 데이터가 선호하는 **스토리지 솔루션**인 경우가 많음
- 따라서 스트리밍 서비스([[Amazon Kinesis|아마존 키네시스]], [[Apache Kafka|아파치 카프카]])가
	- 서로 다른 소스 위치에서 비구조화 및 반구조화 데이터를 수집하여 데이터를 저장한 다음
	- 초기 수준의 운영 변환에 의존하여 이 데이터의 일부를 웨어하우스에서 정형화된 형태로 가져옴
- [[Amazon Kinesis|아마존 키네시스]]용 AWS 람다 함수 또는 카프카 스트림즈용 아파치 카프카 소비자가
	- 이러한 종류의 정규화를 적용하는 대표적인 방법
- 또한 [[AWS 글루]]는 정기적으로 [[data_warehouse|데이터 웨어하우스]]로 이동하는 단계에서 유용