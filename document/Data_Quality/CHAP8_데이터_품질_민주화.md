...
- 더 나아가 데이터 소비, 데이터 품질 규칙의 수, 데이터 셋의 중요도에 관한 다른 핵심 지표들을 기반으로 인사이트 제공
	- 여기에는 데이터가 어떻게 사용되는지, 어떤 데이터가 비즈니스에 가장 중요한지에 관한 내용 포함
- 또한 지식의 전달과 접근성을 한 단계 발전 시키고자 하는 팀 => 누락된 정보나 다른 문맥을 통해 그들만의 데이터 옵스 구축 가능
- 이를 위해 필요할 때뿐만 아니라 평소에도 데이터의 누락된 차원을 추가하도록 권장해야 함
- 지금 당장 사용하지 않는다고 해서 나중에도 영영 쓰지 않는다고 보장할 수 없기 때문


# 8.11. 데이터 거버넌스와 컴플라이언스
- 데이터 품질과 관련된 여러 논의
	- 데이터 거버넌스, 조직 전반과 조직 너머의 데이 관리에 대한 논의 -> 많은 혼란
	- 데이터 거버넌스는 회사의 데이터 리더들이 가장 먼저 떠올리는 주제
	- 최근 데이터 관리 차원에서, [[GDPR]], [[CCPA]], [[IPOs]], 코로나 19 등의 상황으로 데이터 관리에 대한 관심이 특히 더 늘음
	- 그리고 이 밖의 [[데이터 컴플라이언스]]와 개인 정보 보호성의 증가 -> [[데이터 거버넌스]]는 더욱 중요한 주제
- [[데이터 거버넌스]]란 데이터의 가용성, 사용성, 출처, 보안을 유지하는 형태
	- 마치 "CFO를 감옥으로 보내지 않게끔" 하는 일
	- 가트너에 따르면 2022년에 나온 데이터 거버넌스와 관련된 [[데이터 이니셔티브]] 중 80% 이상이 결국 실패할 것이라고 함
- [[데이터 거버넌스]]에 대한 평판이 좋지 않은 주된 이유
	- 클라우드 기반 데이터 스택의 요구 사항에 맞춰 확장하지 못했음
	- 지난 몇 년간 데이터 거버넌스를 위한 도구로 [[데이터 카탈로그]]가 급부상함
	- 데이터 운영이 민주화 됨에 따라, 데이터 웨어 하우스 -> 비즈니스 인텔리전스 플랫폼 -> 카탈로그
	- 데이터 스택의 모든 요소가 컴플라이언스 모범 사례에 발 맞추는게 중요해짐

## 8.11.1. 데이터 카탈로그를 최우선순위로
- 데이터 거버넌스라고 하면 보통 [[데이터 카탈로그]]를 떠올림
	- 데이터 조직이 데이터 사용 및 위치에 관한 메타데이터를 저장하고 선별하는 기반
	- 일반적인 도서관의 카탈로그와 마찬가지
- 데이터 카탈로그는 기업 데이터 소스에 관해 [[단일 진실 공급원]]을 제공함 -> 파이프라인의 데이터 관리 용이
	- 데이터 카탈로그를 사용하면 메타 데이터 저장 및 특정 소스의 계보를 이해하는데 도움
	- 이를 통해 데이터 신뢰도를 높일 수 있음
	- 또한 개인 식별 정보가 어디에 저장되어 어디로 흘러가는지, 이에 접근할 수 있는 권한이 있는 사람이 누구인지 쉽게 추적 가능
- 그동안 데이터 거버넌스를 관리하는 방식
	- 사실상 수동 데이터 카탈로그, 메타데이터 관리 플랫폼
	- 시스템이 발전함에 따라 머신러닝 및 지식 그래프 기반의 기술을 통해 거버넌스의 접근성과 확장성을 높임
- 자동화된 데이터 카탈로그는 크게 세가지
	- 인하우스, 서드파티, 오픈소스 형태

### 인하우스
- 에어비엔비, 넷플릭스, 우버와 같은 B2B 기업들은 주, 국가 그리고 GDPR과 같은 경제 연합체의 [[데이터 컴플라이언스]]를 준수하기 위해 자체적으로 [[데이터 카탈로그]]를 구축
	- 장점: 커스텀이 가능한 대시보드를 빠르게 구축해 조직에 가장 필요한 필드 추출 가능
- 우버의 Databook과 같은 인하우스 솔루션
	- 커스터마이징이 빠르지만 시간이 지날수록 가시성, 협업 저하로 이어짐
	- 특히 데이터 계보 이해시 도드라짐
	- 만약 비즈니스의 다른 부문에서 조직의 데이터 테이블을 어떻게 활용하고 있는지 보여줄 수 있는 정확하고 단일한 창구가 있었더라면
		- 컴플라이언스 준수는 훨씬 더 용이했을 것
- 세부적으로 고려할 사항도 문제이나
	- 절대 다수의 기업에서 수백만 달러 규모의 데이터 카탈로그를 구축하기 위해
	- 엔지니어링 리소스를 투입하는 것은 사실상 불가능


## 8.11.2. 카탈로그 너머: 데이터 거버넌스 강화
- 데이터 조직이 성숙해질수록 데이터 카탈로그만으로는 최신 데이터 거버넌스 프로그램의 요구사항 부합 어려움
	- 데이터 거버넌스 격차 줄이는 일 => 막대한 작업
	- 회사에서 어떤 데이터 자산에 접근하고 있는지 이해하고 있지 않으면,
	- 거버넌스 요구사항 준수 및 격차 줄이는 것은 불가능
	- 데이터 계보, [[data_observability|데이터 옵저버빌리티]]는 이러한 격차를 줄이는데 도움이 됨
- 데이터 접근성과 보안 역시 데이터 거버넌스의 중요한 요소
	- 분석 팀 분산 or 민감한 제 3자 정보 이용시 필수
	- 개인 식별 정보 접근을 관리하기 위해 [[#8.11.1. 데이터 카탈로그를 최우선순위로#인하우스|인하우스]], 서드파티 솔루션 등을 활용하여 자동화, 분산된 정책 시행 필요
- 그러나 [[데이터 카탈로그]], [[data_observability|데이터 옵저버빌리티]] 플랫폼, 데이터 계보, 데이터 접근 관리 체계를 갖추더라도
	- 올바른 절차, 워크플로를 통해 데이터를 신뢰할 수 있고, 안전하게 관리하는 일을 우선시하는 문화 없이는
	- 거버넌스에 대한 지지를 얻기 불가능
- 해당 문화를 구축하기 쉽지 않았던 사례: HubSpot의 BI 담당자 Zosia Kossowski
	- 회사의 급진적인 성장, 데이터 뿐 아니라 구성원들이 일정 수준 자율성을 갖는데 익숙해질 수록 문화적인 차원의 [[데이터 거버넌스]] 갖추기 어려움
		- 회사가 커질수록 더 많은 절차, 규정을 통해 모두를 참여시켜야 함
		- 거버넌스의 부재와 참여 부진이 일으킬 수 있는 문제 경고 필요
	- 데이터 거버넌스 -> 문화적인 변화를 의미
		- A 기업: 데이터를 기반으로 의사결정을 함: 프로덕트 출시전 데이터 위생, 가용성을 우선으로 삼고, 관련 기준을 충족하는데 진심
		- B 기업: 반면, 프로덕트를 내놓은 후에야 엔지니어들에게 잘못되었다고 말할 만큼 데이터를 부산물로 취급
		- A 기업이 거버넌스를 만들기 더 용이할 것
	- 엔지니어 리더와 데이터 생산에 관련된 모든 이들이 초기부터 데이터 관련 논의에 참여해 이후 발생할 수 있는 문제 이해가 바람직

# 8.12. 데이터 품질 전략 수립
- 데이터 엔지니어링 및 분석 리더들이 기업에서 데이터 품질 이니셔티브를 추진할 때 거쳐야 하는 중요한 단계

## 8.12.1. 리더가 책임져야 할 데이터 품질
- 조직 내 리더, 이해 관계자들의 동의를 얻으려고 노력하기 전, 데이터 품질 전략의 현주소를 투명하게 드러내기
- 다음 질문에 답할 것
	- 회사에서 수집하고 저장하고 있는 데이터 자산의 품질을 어떻게 측정하는가
	- 데이터 품질 전략과 관련하여 회의할 때 논의할 주요 KPI 또는 목표는 무엇인가
	- 회사 내 다른 파트의 책임자 또는 데이터 사용자의 교차 가능한 참여를 유도할 수 있는가
	- 전략 KPI 및 목표 달성에 책임을 지는 사람은 누구인가
	- KPI를 올바르게 측정하고 목표를 달성하는게 필요한 견제와 균형은 무엇인가
- 데이터 파이프라인의 가시성 확보 -> 높은 수준의 데이터 품질 쉽게 보장
- 데이터 품질의 전략, 점진적인 진행 상황의 투명성 상향 -> 모든 이들이 정보를 제공받고 책임을 질 수 있도록 함

## 8.12.2. 데이터 품질의 KPI 설정
- SLA를 구체적으로 다루기 전, 데이터 라이프사이클의 각 부분과 데이터가 회사에 가치를 제공하는 방식 이해
- 해당 프로세스의 각 단계의 결과물이 데이터 품질과 SLA 측정을 결정할 것
	- [[data_lake|데이터 레이크]]나 [[data_warehouse|데이터 웨어하우스]]에서 수집한 원시데이터는
	- 데이터 웨어하우스에서 렌더링하여 변환한 데이터와는 충족해야 할 요구사항이 다름
- 데이터 품질 측정에만 너무 집중하는 대신 최대한 간소화 하기
	- '정확성' 점수와 같은 모호한 메트릭이나 자체 개발한 메트릭 대신
	- 완전성, 신선도, 정확성, 일관성, 타당성과 같은 [[data_observability|Data Observability]]에서 언급된 분명한 지표 측정
	- 복잡하고 모호한 유형의 프레임워크는 회사의 우선순위를 충족하기 위해 SLA가 변경되면 혼란 초래

## 8.12.3. [[데이터 거버넌스]] 프로그램 진두지휘
- 데이터 품질 프로그램에 착수했지만 리더들을 포함해 누구도 들은 바가 없다면, 조직 전반에 영향을 미칠 수 x
	- 몇몇 기업을 제외하고는 데이터 엔지니어링 맥락에서 [[데이터 거버넌스]]와 관련된 공식 직책 x
- 회사 내 모든 데이터 사용자들이 [[data_quality|데이터 품질]]이 중요한 이유를 알 수 있도록
	- 데이터 품질을 책임지는 담당자가 데이터의 접근, 사용 및 저장의 모범 사례를 통해
		- 다른 사람을 인도하고 안내할 수 있는 프로그램을 개발하기
	- 대신 누구나 쉽게 참여하고 다른 사람에게도 알릴 수 있도록 해야 함
	- 데이터 품질이 마케팅부터 세일즈까지 각각의 직무에 어떻게 영향을 미치는지 설명하고
		- 각자의 팀에 쉽게 공유하여 실행할 수 있도록 만드는 것이 목표
	- 장기적인 전략을 홍보하고 실행하는 동안에
		- 관심을 얻기 위해 단기적이고 빠른 성과에도 초점을 맞춰야 함

## 8.12.4. 데이터 계보 및 거버넌스 툴 자동화
- 데이터 접근 및 app에 대한 [[데이터 컴플라이언스]] 준수 조치가 점점 더 엄격해짐
- 따라서 [[데이터 거버넌스]]의 수단으로 데이터 품질 모니터링을 수동으로 진행하는 것은 부적합
- 수동으로 데이터 품질을 모니터링하는 방법은 지루하고 시간이 많이 소요됨
	- 데이터 기술 스택의 혁신 속도를 따라가기도 부족함
- 대신 빠른 속도로 데이터를 검증, 모니터링, 문제가 발생했을 때 빠르게 알릴 수 있는 자동화 툴에 투자할 것
	- 특히 사용자가 규칙을 지정하는 기능을 추가하면 데이터가 가진 잠재력 증대

## 8.12.5. 커뮤니케이션 계획 수립
- 준비가 완료됨
- 이제 종합적이고 제대로 구성된 커뮤니케이션 프로그램 계획
	- 리더들을 핵심 구성원으로 포함시키고
	- 이해관곚바들이 프로젝트 진행 상황에 대해 동일하게 이해하며
	- [[데이터 스튜어드]]가 프로젝트를 잘 따라갈 수 있도록 만드는 단계 필요
- 이때 커뮤니케이션 계획은 **쌍방향**이어야 하며 결과물에 관련된 모든 사람을 포함시켜야 함
	- 또한 목표를 완수하기까지 작업을 진척시키는 것과
	- 별개로 회의적인 이들에게 여러분의 조직이 상황을 잘 통제하고 있다는 확신을 심어주어야 함
- 결국 데이터 품질 전략은 기업 내 모든 조직이 데이터를 사용할 권한이 있다고 느끼며
	- 그 데이터를 신뢰할 수 있음을 믿게 만드는 것이 목표
	- 따라서 제대로 구성된 종합적인 [[data_quality|데이터 품질]] 전략은
	- 조직을 효과적으로 확장하는 일부터 우수한 데이터 플랫폼을 구축하는 일까지 대부분의 업무에서 큰 차이를 만듦


# 8.13. 마치며
- [[데이터 민주화]]를 달성하는 과정은 문화적인 절차일 뿐만 아니라 기술적인 절차
- 데이터 사용자의 [[RACI]] 매트릭스 중 어디에 속하는 지와 상관 없이,
	- 데이터 실무자로서 성공하기 위해 데이터 품질이 중요한 역할을 함 
- 데이터 품질 민주화에 필요한 핵심 단계
	- 데이터를 소프트웨어 프로덕트처럼 취급하기
	- 데이터의 원천부터 데이터 품질을 우선시 할 수 있는 데이터 팀 꾸리기
	- [[데이터 리터러시]]를 일급 객체로 대우하기
	- [[데이터 거버넌스]]를 확장할 수 있는 절차와 기술 채택하기
- 8장의 이야기들은 모두 고장난 데이터 파이프라인, 의도치 않은 컴플라이언스 위반, 오래된 대시보드 같은 구체적 경험에서 우러나옴
	- 겪어보아야 데이터 품질에 대한 엔지니어링 역량, 기술 리소스 투자 가능
- 점점 더 많은 기업들이 데이터 민주화 이니셔티브를 주도함
	- 이에 데이터 엔지니어, 분석가가 데이터 품질 관련 모범 사례를 일상 업무에 적용할 수 있도록
	- 데이터 신뢰성 엔지니어, 데이터 옵저버빌리티 전문가, 데이터 리터러시 담당자를 고용하고 있음
