...
## 6.3.2. 사고 책임자가 중요한 이유
- 데이터 사고에 대응할 때는 시간이 가장 중요함
- 적절한 프로세스, 약간의 자동화, 조직적 지원으로 무장해야
	- 데이터 파이프라인의 안정성 보장이 가능함


# 6.4. 페이저듀티의 데이터 사고 관리
- 데이터 플랫폼 및 분석 팀(페이저듀티)
- 데이터 다운타임 방지 및 안정적인 데이터 파이프라인을 만들기?

## 6.4.1. 페이저듀티의 데이터옵스 환경
- 미션: 고객이 언제 어디서나 이해하기 쉽고 효율적인 결정을 내릴 수 있는 신뢰할 만한 데이터 제공
- 그 중 가장 중요한 부분: [[데이터 거버넌스]], [[data_quality|데이터 품질]], 보안, 인프라 운영
- 스노우플레이크, 파이프트랜, 세그먼트, 뮬소프트, AWS, 몬테카를로, 데이터브릭스 솔루션 사용
- ML기반 데이터 옵저버빌리티 기능 통합
	- 데이터 파이프라인의 각 단계에서 사고 모니터링 측정 및 해결

## 6.4.2. 페이저듀티의 데이터 문제
- 다른 SaaS 회사와 마찬가지로 페이저듀티는 많은 클라우드 어플리케이션 사용
	- saleforce, marketo, netsuite
- 정형 데이터, 비정형 데이터를 포함하여
	- 다양한 주기로 들어오는 데이터
	- 다양한 단위로 진행되는 **실시간 배치**는 모두 페이저듀티 전체 데이터 생태계의 일부
- 정확한 데이터를 기반으로 보다 신속하게 의사 결정을 내릴 수 있도록 지원
	- 데이터 품질이 최종 사용자의 기대치를 충족하도록 관리
- **모든 것이 데이터 중심으로 민첩하게 움직여야 함**

## 6.4.3. 데브옵스 우수 사례를 통한 데이터 사고 관리 확장

### 우수 사례 1: 사고 관리를 전체 데이터 라이프사이클에 적용한다
- 페이저듀티의 DE 사고 관리: [[dev_ops]]의 확장이라 할 수 있는 데이터 운영
- 데이터 자체 및 데이터 파이프라인 문제 모두에 대한 추적. 응답 및 문제 분류 포함
- 데이터가 고객 대상 보고서에 표시될 때까지 [[data_warehouse|데이터 웨어하우스]]에 보관되면
	- 누락된 데이터 모델에서 모델 오류에 이르기까지
	- 다양한 유형의 [[data_downtime|데이터 다운타임]]이 발생할 수 있음
- 데이터듀티는 데이터 이상, 신선도, 스키마 변경, 메트릭 추세 등을 포함한 데이터 품질 문제를 모니터링
- [[data_warehouse|데이터 웨어하우스]]에서 [[data_quality|데이터 품질]]을 모니터링하고 보장하기 위해서는 [[data_observability]]가 중요함
	- ETL 툴을 통한 맞춤형 데이터 품질 검사를 통해 데이터 파이프라인에 개입 가능
	- **하지만 갈수록 데이터 생태계의 로직, 스크립트 및 기타 요소 관리가 번거로움**
	- **데이터 추세와 관련된 문제는 파이프라인 품질 검사로 식별 불가**

### 우수 사례 2: 사고 관리에는 노이즈 억제가 포함됨
- **데이터 노이즈는 데이터 모니터링 및 이상 탐지 등을 구현할 때의 주요 문제**
- 엔터프라이즈 규모에서 매일 다양한 '경고'가 발생함
	- 이 경고 중 대다수는 데이터의 변화를 나타냄
	- 변화가 반드시 새로운 문제는 아님
- 신호-노이즈 비율로 경보 시스템의 자동 가능성 판단 가능
	- 가능한 이 비율을 0에 가깝도록 만듦
- 데이터 조직은 고객과 비즈니스 오너를 구분하고
	- 이러한 경고에 적시에 대응하는 동시에 데이터 프로덕트 자체의 소유권 위임 필요
- 여러 데이터 문제가 포함된 하나의 사고에 대해 **여러 번 알람이 발생하지 않도록 통제** 필요
	- **알림에 압도되지 않고** 당면한 데이터 문제 파악 필요

### 우수 사례 3: 데이터 자산 및 사고를 그룹화하여 경고를 지능적으로 발송
- [[data_observability]]는 사고 대응 및 에스컬레이션을 포함한 데이터 사고 관리 단계가 발생하기 전 첫 관문
- '데이터가 업데이트되지 않음' 문제는 비정상적인 추세, 메트릭과 비교할때와 완전히 다른 문제
- 데이터 문제를 그룹화하여 360도 뷰를 기반으로 발송, 알림을 더 쉽게 수행하는 [[data_ops]]의 우수 사례에 따라 작업
- [[data_observability]]를 활용하여
	- 데이터 파이프라인 문제 그룹화
	- 이 워크플로 기반으로 분류 및 경고 기능 구현
	- 경고가 데이터듀티 팀에 올바르게 전달되도록
- 스케줄링에는 airflow를 활용
- [[data_observability]]의 일환으로 경영진 보고 및 재무 보고서를 위한 데이터를 포함하여
	- **회사의 가장 중요한 데이터 자산** 정의
	- **해당 자산과 관련된 경고 발생**
	- **BI**팀에도 전달
- 고객 수, 계정 수, 데이터 사고 수와 같은 BI Metric 상태 모니터링 및 알람 발송 => 데이터 신뢰성 증가
	- 해당 알람은 BI팀에 전달되므로 필요에따라 해당 팀에서 모니터링 및 조치 가능
- [[data_observability|Data Observability]] 원칙을 기반으로 [[dev_ops]] 관점에서 데이터 사고 관리에 접근함

## 6.5. 마치며
- 대규모로 손상된 데이터 파이프라인을 고치는 작업을 할 경우
	- 데이터 조직은 **사고 관리 작업을 반복**할 수 있도록 만들어야 함
	- 그리고 **근본 원인**을 분석해야 함
		- 즉, 데이터 신뢰성 워크플로에 투자해야 함
- 4가지로 요약
	- 사고 관리 프로그램 도입: 중요한 데이터 파이프라인 관리를 위해
	- 이상 탐지 활용: 대규모 사고 감지 전략
	- 사고 발생시 근본 원인 분석 및 영향 분석 철저히 수행
	- 예방 작업: 테스트, CI/CD, [[data_observability|Data Observability]], 데이터 관리를 통한 데이터 품질 문제 예방
